 ...
 * Lines marked as GRR are written by Ruslan Goncharov rus.gon@gmail.com 02-oct-2012
 ...
 * IRrecv::decodeSAMSUNG uses inner parameter kShrink = .90;
 */
...
decoded_value  decValue;//GRR for debugging
...
void IRsend::sendJVC(unsigned long data, int nbits, int repeating )
{ 
 enableIROut(38); 
 data = data << (32 - nbits); 
 if (!repeating )
...
// GRR - kShrink "shrinks" time-length of transmitted logical "1" and "0". Must be approx 0.8
void IRsend::sendSamsung(unsigned long data, int nbits, float kShrink)
{
enableIROut(38);
mark(SAMSUNG_HDR_MARK*kShrink);
space(SAMSUNG_HDR_SPACE*kShrink);
for (int i = 0; i < nbits; i++) {
if (data & TOPBIT) {
mark(SAMSUNG_BIT_MARK*kShrink);
space(SAMSUNG_ONE_SPACE*kShrink);
} 
else {
mark(SAMSUNG_BIT_MARK*kShrink);
space(SAMSUNG_ZERO_SPACE*kShrink);
}
data <<= 1;
}
mark(SAMSUNG_BIT_MARK*kShrink);
...
IRrecv::IRrecv(int recvpin)
{
  ...
  irparams.blinkflag = 1; // GRR
}


// initialization
void IRrecv::enableIRIn() {
  // GRR - Use Timer1 (16bit) --->
  cli();// disable interrupts
  
  //WGM11	WGM10	Description
  //  0	       0	pwm disabled
  TCCR1A = 0;  // Регистр управления А таймера/счетчика1 - TCCR1A - (Timer/Counter1 Control Register A)
  ...
  cbi(TCCR1B,CS12);// 0 - clear
  sbi(TCCR1B,CS11);// 1 - set
  cbi(TCCR1B,CS10);// 0 - clear
  
  //Timer/Counter1 Interrupt Mask Register. It controls which interrupts the timer can trigger. 
  //Setting the TOIE1 bit tells the timer to trigger an interrupt when the timer overflows.
  //sbi(TIMSK1,TOIE1);
  TIMSK1 = _BV(TOIE1);
  RESET_TIMER1;//TCNT1
...
//ISR(TIMER2_OVF_vect)
ISR(TIMER1_OVF_vect) // GRR - Use TIMER1 for receiving
{
  //RESET_TIMER2;
  RESET_TIMER1;// GRR - Use TIMER1 for reciving
...
//GRR for debugging
unsigned long IRrecv::getDecodedValue()
{
